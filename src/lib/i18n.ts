export type Language = 'en' | 'es' | 'pt';

export const translations = {
    en: {
        common: {
            dashboard: "Dashboard",
            board: "Board",
            roadmap: "Versions",
            processes: "Processes",
            apps: "Projects",
            schedule: "Planning",
            settings: "Settings",
            collapse: "Collapse",
            search: "Search...",
            notifications: "Notifications",
            profile: "Profile",
            logout: "Log out",
            new_task: "New Task",
            version_name: "Version Name",
            new_version: "New Version",
            new_app: "New App",
            new_doc: "New Doc",
            edit: "Edit",
            delete: "Delete",
            save: "Save",
            cancel: "Cancel",
            create: "Create",
            status: "Status",
            priority: "Priority",
            project: "Project",
            version: "Version",
            assignee: "Assignee",
            actions: "Actions",
            open_menu: "Open menu",
            loading: "Loading...",
            success: "Success",
            error: "Error",
            all_projects: "All Projects",
            all_versions: "All Versions",
            no_version_filter: "No Version",
            select_project: "Select Project",
            select_language: "Select Language",
            title: "Title",
            description: "Description",
            type: "Type",
            us: "User Story",
            bug: "Bug",
            none: "None",
            unassigned: "Unassigned",
            unknown: "Unknown",
            are_you_sure: "Are you sure?",
            confirm: "Confirm",
            delete_desc: "This action cannot be undone. Are you sure you want to proceed?",
            owner: "Owner",
            selected: "selected"
        },
        dashboard: {
            quick_stats: "Quick Stats",
            total_tasks: "Total Tasks",
            completed_tasks: "Completed Tasks",
            active_versions: "Active Versions",
            recent_activity: "Recent Activity",
            quick_actions: "Quick Actions",
            plan_version: "Plan Version",
            write_doc: "Write Doc",
            welcome_back: "Welcome back",
            heres_update: "Here's what's happening with your projects today."
        },
        board: {
            title: "Kanban Board",
            ideas: "Ideas",
            backlog: "Backlog",
            in_progress: "In Progress",
            code_review: "Code Review",
            done: "Done",
            deployed: "Deployed",
            filter_tasks: "Filter tasks...",
            no_tasks: "No tasks found",
            create_task: "Create Task",
            edit_task: "Edit Task",
            low: "Low",
            medium: "Medium",
            high: "High",
            critical: "Critical",
            view: "Columns",
            toggle_columns: "Toggle Columns",
            clear_filters: "Clear Filters",
            move_to: "Move to..."
        },
        roadmap: {
            title: "Versions",
            subtitle: "Manage project versions and releases",
            version_name: "Version Name",
            release_date: "Release Date",
            release_date_optional: "Release Date (Optional)",
            description_optional: "Description (Optional)",
            planned: "Planned",
            in_development: "In Development",
            in_stores: "In Stores",
            deprecated: "Deprecated",
            create_version: "Create Version",
            create_desc: "Add a new version release to Versions.",
            no_versions: "No versions found. Create one to get started.",
            version_created: "Version created successfully",
            version_deleted: "Version deleted",
            load_error: "Failed to load data",
            create_error: "Failed to create version",
            delete_error: "Failed to delete",
            edit_version: "Edit Version",
            edit_desc: "Update version details.",
            version_updated: "Version updated successfully",
            update_error: "Failed to update version",
            view_list: "List",
            view_board: "Board",
            tbd: "TBD"
        },
        schedule: {
            title: "Planning",
            subtitle: "Month, week and day: the rhythm that drives the Journey.",
            all: "All",
            month: "Month",
            week: "Week",
            day: "Day",
            todo: "To Do",
            doing: "In Progress",
            done: "Done",
            new_item: "+ new",
            new_item_placeholder: "Item title...",
            add_subtask: "+ add subtask",
            subtask_placeholder: "Subtask...",
            date_label: "Date",
            clear_date: "Clear",
            today: "Today",
            all_dates: "All dates",
            scope: "Scope",
            move_to: "Move to",
            day_notes: "Day Notes",
            notes_placeholder: "Reflections, blockers, learnings...",
            empty_column: "—",
            assign_to: "Assign to",
            item_created: "Item created",
            item_updated: "Item updated",
            item_deleted: "Item deleted",
            load_error: "Failed to load schedule",
            save_error: "Failed to save"
        },
        apps: {
            title: "Apps & Projects",
            subtitle: "Manage your projects",
            name: "Name",
            prefix: "Prefix",
            creation_date: "Creation Date",
            color: "Color",
            create_app: "Create Project",
            create_desc: "Add a new project to your workspace.",
            edit_app: "Edit Project",
            edit_desc: "Update project details.",
            no_apps: "No projects found. Create one to get started."
        },
        processes: {
            documents: "Documents",
            process_flows: "Process Flows",
            untitled: "Untitled",
            document: "Document",
            process: "Process",
            select_doc: "Select a document",
            select_doc_desc: "Select a document from the sidebar to view or edit its contents.",
            new_doc_title: "Create Document",
            new_doc_desc: "Create a new text document or process flow.",
            doc_title: "Title",
            doc_type: "Type",
            enter_title: "Enter title:",
            no_docs_title: "No document selected",
            no_docs_desc: "Select a document or create a new one",
            title_main: "Processes & Docs",
            doc_btn: "Doc",
            flow_btn: "Flow",
            created: "Document created",
            saved: "Changes saved successfully",
            deleted: "Document deleted",
            create_error: "Failed to create",
            save_error: "Failed to save",
            delete_error: "Failed to delete",
            placeholder: "Type content here...",
            type_document: "Document",
            type_process: "Process Flow",
            rename_doc_title: "Rename Document",
            rename_doc_desc: "Enter a new title for this document:",
            rename_doc_placeholder: "Document Title",
            rename_node_title: "Rename Node",
            rename_node_desc: "Enter a new label for this node:",
            rename_node_placeholder: "Node Label",
            renamed_success: "Document renamed successfully",
            rename_error: "Failed to rename document"
        },
        settings: {
            title: "Settings",
            subtitle: "Manage your workspace preferences",
            profile: "Profile",
            profile_desc: "Manage your account details and preferences",
            notifications: "Notifications",
            notifications_desc: "Configure how you receive updates",
            appearance: "Appearance",
            appearance_desc: "Customize the look and feel",
            shortcuts: "Keyboard Shortcuts",
            shortcuts_desc: "View and customize shortcuts",
            data: "Data & Storage",
            data_desc: "Manage your data and connected services",
            security: "Security",
            security_desc: "Password, 2FA, and account security",
            danger_zone: "Danger Zone",
            danger_desc: "Irreversible and destructive actions",
            delete_account: "Delete Account",
            rights: "All rights reserved.",
            badges: {
                dark: "Dark",
                enabled: "Enabled"
            }
        },
        team: {
            title: "Team Management",
            subtitle: "Manage your team members and roles.",
            search_placeholder: "Search team...",
            add_member: "Add Member",
            member: "Member",
            nickname: "Nickname",
            role: "Role",
            actions: "Actions",
            no_members: "No members found.",
            add_dialog_title: "Add Team Member",
            add_dialog_desc: "Add a new member to your team.",
            full_name: "Full Name",
            nickname_optional: "Nickname (Optional)",
            role_optional: "Role (Optional)",
            adding: "Adding...",
            remove_dialog_title: "Remove Member",
            remove_dialog_desc: "Are you sure you want to remove this member? They will lose access to the organization immediately.",
            member_added: "Team member added.",
            member_removed: "Team member removed.",
            add_error: "Failed to add member.",
            remove_error: "Failed to remove member.",
            menu: "Team",
            invite_member: "Invite Member",
            email_placeholder: "colleague@company.com",
            pending: "Pending",
            invite_dialog_title: "Invite Team Member",
            invite_dialog_desc: "Send an email invitation to join your team.",
            invite_sent: "Invitation sent successfully!",
            invite_error: "Failed to send invitation.",
            cancel_invite: "Cancel Invite",
            copy_invite_link: "Copy Invite Link",
            edit_member: "Edit Member",
            edit_dialog_title: "Edit Team Member",
            edit_dialog_desc: "Update the details of this team member.",
            member_updated: "Team member updated successfully.",
            update_error: "Failed to update member.",
            roles: {
                admin: "Admin",
                member: "Member"
            }
        },
        auth: {
            login_title: "Access VBoards",
            login_subtitle: "Enter your credentials to continue",
            email: "E-mail",
            password: "Password",
            login_button: "Log in",
            no_account: "Don't have an account?",
            create_account: "Create account",
            register_title: "Create account",
            register_subtitle: "Start managing your projects with VBoards",
            full_name: "Full Name",
            org_name: "Organization / Company Name",
            register_button: "Create account",
            has_account: "Already have an account?",
            login_link: "Log in",
            login_success: "Login successful!",
            welcome_back: "Welcome back.",
            login_error: "Error logging in",
            invalid_credentials: "Invalid email or password.",
            register_success: "Account created successfully!",
            redirecting: "Redirecting...",
            register_error: "Error creating account",
            check_email_for_confirmation: "We sent a confirmation link to your email. Please check your inbox.",
            email_already_registered: "This email is already registered."
        }
    },
    pt: {
        common: {
            dashboard: "Dashboard",
            board: "Quadro",
            roadmap: "Versões",
            processes: "Processos",
            apps: "Projetos",
            schedule: "Planejamento",
            settings: "Configurações",
            collapse: "Recolher",
            search: "Buscar...",
            notifications: "Notificações",
            profile: "Perfil",
            logout: "Sair",
            new_task: "Nova Tarefa",
            version_name: "Nome da Versão",
            new_version: "Nova Versão",
            new_app: "Novo App",
            new_doc: "Novo Doc",
            edit: "Editar",
            delete: "Excluir",
            save: "Salvar",
            cancel: "Cancelar",
            create: "Criar",
            status: "Status",
            priority: "Prioridade",
            project: "Projeto",
            version: "Versão",
            assignee: "Responsável",
            actions: "Ações",
            open_menu: "Abrir menu",
            loading: "Carregando...",
            success: "Sucesso",
            error: "Erro",
            all_projects: "Todos os Projetos",
            all_versions: "Todas as Versões",
            no_version_filter: "Nenhuma Versão",
            select_project: "Selecionar Projeto",
            select_language: "Selecionar Idioma",
            title: "Título",
            description: "Descrição",
            type: "Tipo",
            us: "História",
            bug: "Bug",
            none: "Nenhum",
            unassigned: "Não atribuído",
            unknown: "Desconhecido",
            are_you_sure: "Tem certeza?",
            confirm: "Confirmar",
            delete_title: "Excluir item?",
            delete_desc: "Esta ação não pode ser desfeita. Tem certeza de que deseja prosseguir?",
            owner: "Dono",
            selected: "selecionados"
        },
        dashboard: {
            quick_stats: "Estatísticas Rápidas",
            total_tasks: "Total de Tarefas",
            completed_tasks: "Tarefas Concluídas",
            active_versions: "Versões Ativas",
            recent_activity: "Atividade Recente",
            quick_actions: "Ações Rápidas",
            plan_version: "Planejar Versão",
            write_doc: "Escrever Doc",
            welcome_back: "Bem-vindo de volta",
            heres_update: "Aqui está o que está acontecendo com seus projetos hoje."
        },
        board: {
            title: "Quadro Kanban",
            ideas: "Ideias",
            backlog: "Backlog",
            in_progress: "Em Progresso",
            code_review: "Revisão de Código",
            done: "Concluído",
            deployed: "Implantado",
            filter_tasks: "Filtrar tarefas...",
            no_tasks: "Nenhuma tarefa encontrada",
            create_task: "Criar Tarefa",
            edit_task: "Editar Tarefa",
            low: "Baixa",
            medium: "Média",
            high: "Alta",
            critical: "Crítica",
            view: "Colunas",
            toggle_columns: "Alterar Colunas",
            clear_filters: "Limpar Filtros",
            move_to: "Mover para..."
        },
        roadmap: {
            title: "Versões",
            subtitle: "Gerencie versões e lançamentos do projeto",
            version_name: "Nome da Versão",
            release_date: "Data de Lançamento",
            release_date_optional: "Data de Lançamento (Opcional)",
            description_optional: "Descrição (Opcional)",
            planned: "Previsto",
            in_development: "Em Desenvolvimento",
            in_stores: "Nas Lojas",
            deprecated: "Descontinuado",
            create_version: "Criar Versão",
            create_desc: "Adicionar uma nova versão.",
            no_versions: "Nenhuma versão encontrada. Crie uma para começar.",
            version_created: "Versão criada com sucesso",
            version_deleted: "Versão excluída",
            load_error: "Falha ao carregar dados",
            create_error: "Falha ao criar versão",
            delete_error: "Falha ao excluir",
            edit_version: "Editar Versão",
            edit_desc: "Atualizar detalhes da versão.",
            version_updated: "Versão atualizada com sucesso",
            update_error: "Falha ao atualizar versão",
            view_list: "Lista",
            view_board: "Quadro",
            tbd: "A definir"
        },
        schedule: {
            title: "Planejamento",
            subtitle: "Mês, semana e dia: o ritmo que move a Trajetória.",
            all: "Todos",
            month: "Mês",
            week: "Semana",
            day: "Dia",
            todo: "A Fazer",
            doing: "Em Andamento",
            done: "Feito",
            new_item: "+ nova",
            new_item_placeholder: "Título do item...",
            add_subtask: "+ adicionar subtarefa",
            subtask_placeholder: "Subtarefa...",
            date_label: "Data",
            clear_date: "Limpar",
            today: "Hoje",
            all_dates: "Todas as datas",
            scope: "Escopo",
            move_to: "Mover para",
            day_notes: "Notas do Dia",
            notes_placeholder: "Reflexões, bloqueios, aprendizados...",
            empty_column: "—",
            assign_to: "Atribuir para",
            item_created: "Item criado",
            item_updated: "Item atualizado",
            item_deleted: "Item excluído",
            load_error: "Falha ao carregar o planejamento",
            save_error: "Falha ao salvar"
        },
        apps: {
            title: "Apps e Projetos",
            subtitle: "Gerencie seus projetos",
            name: "Nome",
            prefix: "Prefixo",
            creation_date: "Data de Criação",
            color: "Cor",
            create_app: "Criar Projeto",
            create_desc: "Adicionar um novo projeto a sua área de trabalho.",
            edit_app: "Editar Projeto",
            edit_desc: "Atualizar detalhes do projeto.",
            no_apps: "Nenhum projeto encontrado. Crie um para começar."
        },
        processes: {
            documents: "Documentos",
            process_flows: "Fluxos de Processo",
            untitled: "Sem Título",
            document: "Documento",
            process: "Processo",
            select_doc: "Selecione um documento",
            select_doc_desc: "Selecione um documento da barra lateral para visualizar ou editar seu conteúdo.",
            new_doc_title: "Criar Documento",
            new_doc_desc: "Crie um novo documento de texto ou fluxo de processo.",
            doc_title: "Título",
            doc_type: "Tipo",
            enter_title: "Digite o título:",
            no_docs_title: "Nenhum documento selecionado",
            no_docs_desc: "Selecione um documento ou crie um novo",
            title_main: "Processos e Docs",
            doc_btn: "Doc",
            flow_btn: "Fluxo",
            created: "Documento criado",
            saved: "Alterações salvas com sucesso",
            deleted: "Documento excluído",
            create_error: "Falha ao criar",
            save_error: "Falha ao salvar",
            delete_error: "Falha ao excluir",
            placeholder: "Digite o conteúdo aqui...",
            type_document: "Documento",
            type_process: "Fluxo de Processo",
            rename_doc_title: "Renomear Documento",
            rename_doc_desc: "Digite um novo título para este documento:",
            rename_doc_placeholder: "Título do Documento",
            rename_node_title: "Renomear Nó",
            rename_node_desc: "Digite um novo rótulo para este nó:",
            rename_node_placeholder: "Rótulo do Nó",
            renamed_success: "Documento renomeado com sucesso",
            rename_error: "Falha ao renomear documento"
        },

        settings: {
            title: "Configurações",
            subtitle: "Gerencie as preferências do seu workspace",
            profile: "Perfil",
            profile_desc: "Gerencie detalhes e preferências da conta",
            notifications: "Notificações",
            notifications_desc: "Configure como você recebe atualizações",
            appearance: "Aparência",
            appearance_desc: "Personalize a aparência",
            shortcuts: "Atalhos de Teclado",
            shortcuts_desc: "Visualize e personalize atalhos",
            data: "Dados e Armazenamento",
            data_desc: "Gerencie seus dados e serviços conectados",
            security: "Segurança",
            security_desc: "Senha, 2FA e segurança da conta",
            danger_zone: "Zona de Perigo",
            danger_desc: "Ações irreversíveis e destrutivas",
            delete_account: "Excluir Conta",
            rights: "Todos os direitos reservados.",
            badges: {
                dark: "Escuro",
                enabled: "Ativo"
            }
        },
        team: {
            title: "Gestão de Equipe",
            subtitle: "Gerencie os membros e funções da sua equipe.",
            search_placeholder: "Buscar equipe...",
            add_member: "Adicionar Membro",
            member: "Membro",
            nickname: "Apelido",
            role: "Função",
            actions: "Ações",
            no_members: "Nenhum membro encontrado.",
            add_dialog_title: "Adicionar Membro da Equipe",
            add_dialog_desc: "Adicione um novo membro à sua equipe.",
            full_name: "Nome Completo",
            nickname_optional: "Apelido (Opcional)",
            role_optional: "Função (Opcional)",
            adding: "Adicionando...",
            remove_dialog_title: "Remover Membro",
            remove_dialog_desc: "Tem certeza de que deseja remover este membro? Ele perderá o acesso à organização imediatamente.",
            member_added: "Membro da equipe adicionado.",
            member_removed: "Membro da equipe removido.",
            add_error: "Falha ao adicionar membro.",
            remove_error: "Falha ao remover membro.",
            menu: "Equipe",
            invite_member: "Convidar Membro",
            email_placeholder: "colega@empresa.com",
            pending: "Pendente",
            invite_dialog_title: "Convidar Membro",
            invite_dialog_desc: "Envie um convite por e-mail para participar da sua equipe.",
            invite_sent: "Convite enviado com sucesso!",
            invite_error: "Falha ao enviar convite.",
            cancel_invite: "Cancelar Convite",
            copy_invite_link: "Copiar Link de Convite",
            edit_member: "Editar Membro",
            edit_dialog_title: "Editar Membro da Equipe",
            edit_dialog_desc: "Atualize os detalhes deste membro da equipe.",
            member_updated: "Membro da equipe atualizado com sucesso.",
            update_error: "Falha ao atualizar membro.",
            roles: {
                admin: "Administrador",
                member: "Membro"
            }
        },
        auth: {
            login_title: "Acessar VBoards",
            login_subtitle: "Entre com suas credenciais para continuar",
            email: "E-mail",
            password: "Senha",
            login_button: "Entrar",
            no_account: "Não tem uma conta?",
            create_account: "Criar conta",
            register_title: "Criar conta",
            register_subtitle: "Comece a gerenciar seus projetos com o VBoards",
            full_name: "Nome Completo",
            org_name: "Nome da Organização / Empresa",
            register_button: "Criar conta",
            has_account: "Já tem uma conta?",
            login_link: "Login",
            login_success: "Login realizado!",
            welcome_back: "Bem-vindo de volta.",
            login_error: "Erro ao entrar",
            invalid_credentials: "E-mail ou senha inválidos.",
            register_success: "Conta criada com sucesso!",
            redirecting: "Redirecionando...",
            register_error: "Erro ao criar conta",
            check_email_for_confirmation: "Enviamos um link de confirmação para seu e-mail. Por favor, verifique sua caixa de entrada.",
            email_already_registered: "Este e-mail já está cadastrado."
        }
    },
    es: {
        common: {
            dashboard: "Panel",
            board: "Tablero",
            roadmap: "Hoja de Ruta",
            processes: "Procesos",
            apps: "Proyectos",
            schedule: "Planificación",
            settings: "Ajustes",
            collapse: "Colapsar",
            search: "Buscar...",
            notifications: "Notificaciones",
            profile: "Perfil",
            logout: "Cerrar sesión",
            new_task: "Nueva Tarea",
            version_name: "Nombre de la Versión",
            new_version: "Nueva Versión",
            new_app: "Nueva App",
            new_doc: "Nuevo Doc",
            edit: "Editar",
            delete: "Eliminar",
            save: "Guardar",
            cancel: "Cancelar",
            create: "Crear",
            status: "Estado",
            priority: "Prioridad",
            project: "Proyecto",
            version: "Versión",
            assignee: "Asignado",
            actions: "Acciones",
            open_menu: "Abrir menú",
            loading: "Cargando...",
            success: "Éxito",
            error: "Error",
            all_projects: "Todos los Proyectos",
            all_versions: "Todas las Versiones",
            no_version_filter: "Sin Versión",
            select_project: "Seleccionar Proyecto",
            select_language: "Seleccionar Idioma",
            title: "Título",
            description: "Descripción",
            type: "Tipo",
            us: "Historia",
            bug: "Bug",
            none: "Ninguno",
            unassigned: "Sin Asignar",
            unknown: "Desconocido",
            are_you_sure: "¿Estás seguro?",
            confirm: "Confirmar",
            delete_title: "¿Eliminar elemento?",
            delete_desc: "Esta acción no se puede deshacer. ¿Estás seguro de que quieres continuar?",
            owner: "Propietario",
            selected: "seleccionados"
        },
        dashboard: {
            quick_stats: "Estadísticas Rápidas",
            total_tasks: "Tareas Totales",
            completed_tasks: "Tareas Completadas",
            active_versions: "Versiones Activas",
            recent_activity: "Actividad Reciente",
            quick_actions: "Acciones Rápidas",
            plan_version: "Planificar Versión",
            write_doc: "Escribir Doc",
            welcome_back: "Bienvenido de nuevo",
            heres_update: "Aquí está lo que sucede con tus proyectos hoy."
        },
        board: {
            title: "Tablero Kanban",
            ideas: "Ideas",
            backlog: "Backlog",
            in_progress: "En Progreso",
            code_review: "Revisión de Código",
            done: "Hecho",
            deployed: "Desplegado",
            filter_tasks: "Filtrar tareas...",
            no_tasks: "No se encontraron tareas",
            create_task: "Crear Tarea",
            edit_task: "Editar Tarea",
            low: "Baja",
            medium: "Media",
            high: "Alta",
            critical: "Crítica",
            view: "Columnas",
            toggle_columns: "Cambiar columnas",
            clear_filters: "Limpiar Filtros",
            move_to: "Mover a..."
        },
        roadmap: {
            title: "Hoja de Ruta",
            subtitle: "Gestionar versiones y lanzamientos del proyecto",
            version_name: "Nombre de la Versión",
            release_date: "Fecha de Lanzamiento",
            release_date_optional: "Fecha de Lanzamiento (Opcional)",
            description_optional: "Descripción (Opcional)",
            planned: "Planificado",
            in_development: "En Desarrollo",
            in_stores: "En Tiendas",
            deprecated: "Obsoleto",
            create_version: "Crear Versión",
            create_desc: "Añadir una nueva versión a la hoja de ruta.",
            no_versions: "No se encontraron versiones. Crea una para empezar.",
            version_created: "Versión creada con éxito",
            version_deleted: "Versión eliminada",
            load_error: "Error al cargar datos",
            create_error: "Error al crear versión",
            delete_error: "Error al eliminar",
            edit_version: "Editar Versión",
            edit_desc: "Actualizar detalles de la versión.",
            version_updated: "Versión actualizada con éxito",
            update_error: "Error al actualizar versión",
            view_list: "Lista",
            view_board: "Tablero",
            tbd: "Por definir"
        },
        schedule: {
            title: "Planificación",
            subtitle: "Mes, semana y día: el ritmo que mueve la Trayectoria.",
            all: "Todos",
            month: "Mes",
            week: "Semana",
            day: "Día",
            todo: "Por Hacer",
            doing: "En Progreso",
            done: "Hecho",
            new_item: "+ nueva",
            new_item_placeholder: "Título del elemento...",
            add_subtask: "+ añadir subtarea",
            subtask_placeholder: "Subtarea...",
            date_label: "Fecha",
            clear_date: "Limpiar",
            today: "Hoy",
            all_dates: "Todas las fechas",
            scope: "Ámbito",
            move_to: "Mover a",
            day_notes: "Notas del Día",
            notes_placeholder: "Reflexiones, bloqueos, aprendizajes...",
            empty_column: "—",
            assign_to: "Asignar a",
            item_created: "Elemento creado",
            item_updated: "Elemento actualizado",
            item_deleted: "Elemento eliminado",
            load_error: "Error al cargar la planificación",
            save_error: "Error al guardar"
        },
        apps: {
            title: "Apps y Proyectos",
            subtitle: "Gestiona tus proyectos",
            name: "Nombre",
            prefix: "Prefijo",
            creation_date: "Fecha de Creación",
            color: "Color",
            create_app: "Crear Proyecto",
            create_desc: "Añadir un nuevo proyecto a tu área de trabajo.",
            edit_app: "Editar Proyecto",
            edit_desc: "Actualizar detalles del proyecto.",
            no_apps: "No se encontraron proyectos. Crea uno para empezar."
        },
        processes: {
            documents: "Documentos",
            process_flows: "Flujos de Proceso",
            untitled: "Sin Título",
            document: "Documento",
            process: "Proceso",
            select_doc: "Selecciona un documento",
            select_doc_desc: "Selecciona un documento de la barra lateral para ver o editar.",
            new_doc_title: "Crear Documento",
            new_doc_desc: "Crea un nuevo documento de texto o flujo de proceso.",
            doc_title: "Título",
            doc_type: "Tipo",
            enter_title: "Introduce el título:",
            no_docs_title: "No seleccionado",
            no_docs_desc: "Selecciona un documento o crea uno nuevo",
            title_main: "Procesos y Documentos",
            doc_btn: "Doc",
            flow_btn: "Flujo",
            created: "Documento creado",
            saved: "Cambios guardados con éxito",
            deleted: "Documento eliminado",
            create_error: "Error al crear",
            save_error: "Error al guardar",
            delete_error: "Error al eliminar",
            placeholder: "Escribe el contenido aquí...",
            type_document: "Documento",
            type_process: "Flujo de Proceso",
            rename_doc_title: "Renombrar Documento",
            rename_doc_desc: "Introduce un nuevo título para este documento:",
            rename_doc_placeholder: "Título del Documento",
            rename_node_title: "Renombrar Nodo",
            rename_node_desc: "Introduce una nueva etiqueta para este nodo:",
            rename_node_placeholder: "Etiqueta del Nodo",
            renamed_success: "Documento renombrado con éxito",
            rename_error: "Error al renombrar documento"
        },
        settings: {
            title: "Configuración",
            subtitle: "Administra las preferencias de tu espacio de trabajo",
            profile: "Perfil",
            profile_desc: "Administra los detalles y preferencias de la cuenta",
            notifications: "Notificaciones",
            notifications_desc: "Configura cómo recibes actualizaciones",
            appearance: "Apariencia",
            appearance_desc: "Personaliza la apariencia",
            shortcuts: "Atajos de Teclado",
            shortcuts_desc: "Ver y personalizar atajos",
            data: "Datos y Almacenamiento",
            data_desc: "Administra tus datos y servicios conectados",
            security: "Seguridad",
            security_desc: "Contraseña, 2FA y seguridad de la cuenta",
            danger_zone: "Zona de Peligro",
            danger_desc: "Acciones irreversibles y destructivas",
            delete_account: "Eliminar Cuenta",
            rights: "Todos los derechos reservados.",
            badges: {
                dark: "Oscuro",
                enabled: "Habilitado"
            }
        },
        team: {
            title: "Gestión de Equipo",
            subtitle: "Gestiona los miembros y roles de tu equipo.",
            search_placeholder: "Buscar equipo...",
            add_member: "Añadir Miembro",
            member: "Miembro",
            nickname: "Apodo",
            role: "Rol",
            actions: "Acciones",
            no_members: "No se encontraron miembros.",
            add_dialog_title: "Añadir Miembro del Equipo",
            add_dialog_desc: "Añade un nuevo miembro a tu equipo.",
            full_name: "Nombre Completo",
            nickname_optional: "Apodo (Opcional)",
            role_optional: "Rol (Opcional)",
            adding: "Añadiendo...",
            remove_dialog_title: "Eliminar Miembro",
            remove_dialog_desc: "¿Estás seguro de que deseas eliminar a este miembro? Perderá el acceso a la organización inmediatamente.",
            member_added: "Miembro del equipo añadido.",
            member_removed: "Miembro del equipo eliminado.",
            add_error: "Error al añadir miembro.",
            remove_error: "Error al eliminar miembro.",
            menu: "Equipo",
            invite_member: "Invitar Miembro",
            email_placeholder: "colega@empresa.com",
            pending: "Pendiente",
            invite_dialog_title: "Invitar Miembro",
            invite_dialog_desc: "Envía una invitación por correo para unirse a tu equipo.",
            invite_sent: "¡Invitación enviada con éxito!",
            invite_error: "Error al enviar invitación.",
            cancel_invite: "Cancelar Invitación",
            copy_invite_link: "Copiar Enlace de Invitación",
            edit_member: "Editar Miembro",
            edit_dialog_title: "Editar Miembro del Equipo",
            edit_dialog_desc: "Actualiza los detalles de este miembro del equipo.",
            member_updated: "Miembro del equipo actualizado con éxito.",
            update_error: "Error al actualizar miembro.",
            roles: {
                admin: "Administrador",
                member: "Miembro"
            }
        },
        auth: {
            login_title: "Acceder a VBoards",
            login_subtitle: "Introduce tus credenciales para continuar",
            email: "Correo electrónico",
            password: "Contraseña",
            login_button: "Entrar",
            no_account: "¿No tienes una cuenta?",
            create_account: "Crear cuenta",
            register_title: "Crear cuenta",
            register_subtitle: "Comienza a gestionar tus proyectos con VBoards",
            full_name: "Nombre completo",
            org_name: "Nombre de la Organización / Empresa",
            register_button: "Crear cuenta",
            has_account: "¿Ya tienes una cuenta?",
            login_link: "Iniciar sesión",
            login_success: "¡Inicio de sesión exitoso!",
            welcome_back: "Bienvenido de nuevo.",
            login_error: "Error al iniciar sesión",
            invalid_credentials: "Correo electrónico o contraseña inválidos.",
            register_success: "¡Cuenta creada con éxito!",
            redirecting: "Redireccionando...",
            register_error: "Error al crear la cuenta",
            check_email_for_confirmation: "Hemos enviado um enlace de confirmación a su correo electrónico. Por favor, verifique su bandeja de entrada.",
            email_already_registered: "Este correo electrónico ya está registrado."
        }
    }
};
